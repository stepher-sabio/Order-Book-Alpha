...
// Extract Microsoft data to Parquet format
void extractMicrosoftData(const std::vector<std::string>& files, 
    ...
    for (const auto& file : files) {
        ...
        try {
            ...
            std::cout << file_msft_count << " Microsoft records\n";
    ...
    PARQUET_ASSIGN_OR_THROW(
        outfile,
        arrow::io::FileOutputStream::Open("microsoft_data.parquet"));

    PARQUET_THROW_NOT_OK(
        parquet::arrow::WriteTable(*table, arrow::default_memory_pool(), outfile, 1000000));

    std::cout << "\n✅ EXTRACTION COMPLETE!\n";
    std::cout << "Total Microsoft (MSFT) records: " << total_msft_records << "\n";
    std::cout << "Output file: microsoft_data.parquet\n";
}
...
int main() {
    ...
    // Microsoft's instrument_id is 10888
    uint32_t MSFT_ID = 10888;
    
    std::cout << "\n💼 Extracting Microsoft (instrument_id: " << MSFT_ID << ") data to Parquet...\n\n";
    extractMicrosoftData(files, basePath, MSFT_ID);
